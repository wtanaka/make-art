...
module.exports = function (id, width, height) {
    width = width || 500;
    height = height || 500;
    var windSimulator = function(p) {
      ...
      Particle.prototype.move = function() {
        ...
        if(this.x < -5){ this.x = p.width+4; this.prevx = this.x; this.y = p.random(0,width) ; this.prevy = this.y}
        if(this.y < -5){ this.y = p.height+4;this.prevy = this.y; this.x = p.random(0,height) ; this.prevx = this.x}
        if(this.x > p.width+5){ this.x = -4 ;this.prevx = this.x; this.y = p.random(0,width) ; this.prevy = this.y}
        if(this.y > p.height+5){ this.y = -4 ;this.prevy = this.y; this.x = p.random(0,height) ;this.prevx = this.x}
