...
app.directive('display', function ($window, $timeout, $location, $rootScope, $http) {
    ...
    return {
        ...
        link        : function (scope, element, attrs, workspaceCtl) {
            ...
            function init() {
                ...
                if (scope.mode === 'run-app' || scope.mode === 'run-app-iframe') {
                    var input = document.getElementsByTagName('input')[0],
                        button = document.getElementsByTagName('button')[0];

                    if (input && button) {
                        $http.get('http://paul-kano-geoip.herokuapp.com/city').success(function (res) {
                            console.log('making request');
                            console.log('res', res.City.Names.en);
                            $timeout(function () {
                                document.getElementsByTagName('input')[0].setAttribute('value', res.City.Names.en);
                                //document.getElementsByTagName('button')[0].click();
                                document.querySelector('#toolbar-container-0').dispatchEvent(new Event('updateLocation'));
                            }, 3000);
                        });
             ...
             * @return void
             */
            scope.update = td.throttle(THROTTLE_MS, false, function (oldValue, newValue, h, ui) {
                ...
                timer = setTimeout(function () {
                    ...
                    scope.error = null;
                    var code = scope.source;
                    if (!ui) {
                        settings.pre = scope.trigger.code;
                        document.getElementById('uiDisplay').innerHTML = '';
                    } else {
                        if (scope.trigger.valueName && window.triggerValue) {
                            settings.pre = 'var ' + scope.trigger.valueName + '=\'' + window.triggerValue + '\';\n'
                        }
                    }
            ...
            window.redraw = scope.update.bind(scope, '', '', null);
