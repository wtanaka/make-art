...
app.directive('display', function ($window, $timeout, $rootScope) {
    ...
    return {
        ...
        link        : function (scope, element, attrs, workspaceCtl) {
            ...
            function getMakeDataCode() {

                var rawFile = new XMLHttpRequest(),
                    deferred = q.defer();

                rawFile.open('GET', './assets/make-data.txt', false);
            ...
            scope.update = td.throttle(THROTTLE_MS, false, function () {
                var language = require('../language/index'),
                    settings = scope,
                    query_code = "response = jQuery.getJSON('http://api.openweathermap.org/data/2.5/weather?q=' + query + '&APPID=1691a75f2882233290efe7dccf49459e',decideWeather)",
                    err;

                // Reset error, evaluate code and catch new potentia;errors
                scope.error = null;

                getMakeDataCode().then(function (response) {


                    err = language.run(response + '\n' + scope.source, settings);
            ...
            scope.resize = function () {
                ...
                ctx.scale(scope.canvas.width / k, scope.canvas.height / k);

                console.log('test ----- >', scope.canvas.width, scope.canvas.height);
