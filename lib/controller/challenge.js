...
app.controller('ChallengeController', function ($scope, $routeParams, $window, $timeout, $rootScope, $location, contentService, emailService, socialService) {
    ...
    function loadChallenge() {
        contentService.challenge.get($scope.worldId, $scope.id).then(function (challenge) {
            ...
            $scope.selectedTrigger = triggers.getTrigger(challenge.trigger || 'none');
    ...
    $scope.openShareModal = function () {
        if ($rootScope.creation) {
            // route to the new creation's page
            var url = "http://apps-world.kano.me/shared/" + $rootScope.creation.id;
    ...
    $scope.goToNext = function () {
        if ($rootScope.selectedWorld.share_strategy === 'last_only') {
            if (!$scope.next) {
                $scope.openExportModal();
            } else {
                $scope.openNextModal();
            }
        } else if ($rootScope.selectedWorld.share_strategy && !$rootScope.creation) {
            $scope.openExportModal();
        } else {
            $scope.openNextModal();
        }
